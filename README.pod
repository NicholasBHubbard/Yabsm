...
Takes read only snapshots and performs remote and local incremental backups.
...
As we can see yabsm provides four commands: C<config>, C<find>, C<ssh>, and C<daemon>.
...
I<yabsm> (and a group named I<yabsm>) that the daemon will run as.
...
storing snapshots, and use as a cache for holding data needed for performing
...
The following table describes in English what each timeframe means.
...
running C<yabsm ssh check <SSH_BACKUP>> For a I<ssh_backup> to be able to be
performed the following conditions must be satisfied:
...
If the backup directory (C<dir>) does not exist it will be created when the Yabsm
daemon starts.

=head4 Configuration Query

Yabsm comes with a C<config> command that allows you to query your configuration.
