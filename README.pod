...
As we can see yabsm provides 3 commands: L<config|/"Configuration Querying">,
...
running C<yabsm config ssh-check <SSH_BACKUP>>. For a I<ssh_backup> to be able to
be performed the following conditions must be satisfied:
...
The Yabsm daemon logs all of it's errors to C</var/log/yabsm> so if, for example,
...
The C<ssh-check <SSH_BACKUP>> subcommand checks that C<<SSH_BACKUP>> can be
