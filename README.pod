...
I<snaps>, I<ssh_backups>, and I<local_backups> are configured to use one or more
...
string for the C<find> subcommand.
    ...
    $ yabsm <find|f> <SNAP|SSH_BACKUP|LOCAL_BACKUP> QUERY
...
C<newest> -> Returns the most recent snapshot/backup.

=item 2

C<oldest> -> Returns the oldest snapshot/backup.

=item 3

C<after TIME> -> Returns all the snapshot/backups that are newer than TIME.

=item 4

C<before TIME> -> Returns all the snapshot/backups that are older than TIME.

=item 5

C<between TIME1 TIME2> -> Returns all the snapshot/backups that were taken between
TIME1 and TIME2.

=item 6 

C<TIME> -> Return the snapshot/backup that was taken closest to TIME.

=head2 Time Abbreviations

In the list above the C<TIME> variables stand for a I<time abbreviation>.

There are two different kinds of I<time abbreviations>: I<relative times> and I<immediate times>.
...
C<minutes>, C<hours>, or C<days>. Each I<UNIT> can be abbreviated: 

=over 4

C<minutes> -> C<mins>, C<m>
C<hours>   -> C<hrs>, C<h>
C<days>    -> C<d>
