...
The second argument is a snapshot location query. There are 6 types of queries:
    ...
    C<between TIME1 TIME2> -> Returns all the snapshot/backups that were taken between TIME1 and TIME2.
...
Next lets talk about I<immediate times>. An I<immediate_time> is always an
abbreviation for C<yyyy_mm_dd_hh:mm> (the same format used for yabsm 
snapshot/backup names).
